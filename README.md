...
| **운영위 재정** | `finance_ops` | + 운영위원회 신청 검토/반려 |
| **운영위 승인자** | `approver_ops` | + 운영위원회 신청 최종 승인/반려 |
| **준비위 재정(총괄)** | `finance_prep` | + 모든 위원회 신청 검토/반려, 정산, 승인건 반려, 영수증 관리, 대시보드, 예산 설정, 사용자 관리 |
| **준비위 승인자** | `approver_prep` | + 준비위원회 신청 최종 승인/반려 |
| **위원장** | `director` | + 모든 위원회 신청 최종 승인/반려, 대시보드, 고액 신청 승인 |
| **관리자** | `admin` | + 전체 권한, 사용자 관리, 프로젝트 관리 |
...
2. 전체 사용자 목록이 표시됩니다.
3. 드롭다운으로 권한을 변경할 수 있습니다 (일반/운영위 재정/운영위 승인자/준비위 재정(총괄)/준비위 승인자/위원장/관리자).
...
| 검토완료 (pending→reviewed) | 해당 위원회 승인자 | 승인 요청 |
