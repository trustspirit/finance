...
- 상단 **프로젝트 드롭다운**에서 프로젝트를 전환합니다.
- Admin은 드롭다운에서 프로젝트 **생성** 및 삭제된 프로젝트 **복원**이 가능합니다.
- Admin은 상단 ⚙ 아이콘(`/settings`)에서 프로젝트별 설정(Document No., 예산, 멤버, 승인 금액 기준)을 편집합니다.
- 프로젝트별로 Document No., 예산, 멤버, 승인 금액 기준을 설정합니다.
- 1개 프로젝트만 할당된 일반 사용자는 자동 선택되며 드롭다운이 표시되지 않습니다.
...
2. 삭제된 프로젝트는 상단 프로젝트 드롭다운 하단에 남은 일수와 함께 표시됩니다 (Admin만 표시).
...
1. 상단 우측의 사용자 이름을 클릭하고 **내 설정**(`/profile`)으로 이동합니다.
...
**내 설정** (`/profile`) — 상단 우측의 사용자 이름 클릭 > **내 설정**
- 모든 사용자: 언어, 이름, 연락처, 은행/계좌, 통장사본, 위원회, 서명

**프로젝트 설정** (`/settings`) — 상단 우측의 ⚙ 아이콘 (Admin만 표시)
- 현재 선택된 프로젝트의 설정을 편집합니다 (Document No., 예산, 멤버 관리, 기본 프로젝트 지정, 삭제)
- 프로젝트 전환은 상단 드롭다운에서 수행합니다
...
내 설정(`/profile`)에서 한국어/English를 전환할 수 있습니다. 브라우저 언어를 자동 감지하며, 선택한 언어는 저장됩니다.
